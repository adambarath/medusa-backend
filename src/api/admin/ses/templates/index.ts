...
import { SESOptions } from "../../../../modules/aws-ses/services/awsses-module-service";
...
export default (rootDirectory: string): Router | Router[] => {
  ...
  const {
    configModule: { projectConfig, plugins, modules },
  ...
  const sesConfig: SESOptions = plugins.find(
    (p: any) => p.resolve === "medusa-plugin-ses"
  )["options"];
  ...
  router.post("/admin/ses/templates", async (req, res) => {
    ...
    const sesService = req.scope.resolve("sesService");
    let template = await sesService.createTemplate(
      data.templateId,
      data.subject,
      data.html,
      data.text
  ...
  router.post("/admin/ses/templates/:id", async (req, res) => {
    ...
    const sesService = req.scope.resolve("sesService");
    let template = await sesService.updateTemplate(
      req.params.id,
      data.subject,
      data.html,
      data.text
